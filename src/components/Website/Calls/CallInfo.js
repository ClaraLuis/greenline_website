...
const CallInfo = (props) => {
    ...
    const [success, setsuccess] = useState({ type: '', status: false });
    ...
    const AssigntoPhaseMutation = useMutation('AssigntoPhaseMutation_API', {
        ...
        onSuccess: (data, variables, context) => {
            if (data.data.status) {
                ...
                FetchPhases?.refetch();
                props?.setopenModal(false);
                setsuccess({ type: 'phase', status: true });
                NotificationManager.success('', 'Success');
            } else {
                NotificationManager.warning(data.data.reason, 'Warning');
            }
    ...
    const Updateleadstatus = useMutation('Updateleadstatus_API', {
        ...
        onSuccess: (data, variables, context) => {
            if (data.data.status) {
                props?.FetchLeads.refetch();
                setsuccess({ type: 'lead status', status: true });
                NotificationManager.success('', 'Success');
            } else {
                NotificationManager.warning(data.data.reason, 'Warning');
            }
    ...
    useEffect(() => {
        setTimeout(() => {
            setsuccess({ type: '', status: false });
        }, 15000);
    }, [AssigntoPhaseMutation?.isSuccess, Updateleadstatus?.isSuccess]);
    return (
        <Modal
        ...
        >
            ...
            <Modal.Body style={{ display: props?.dealsmodal || props?.meetingfolowupmodal?.open ? 'none' : '' }}>
                <div class="row m-0 w-100 py-2 ">
                    ...
                    <div
                        //  style={{ borderInlineEnd: '1px solid rgba(0, 0, 0, 0.1)' }}
                        class="col-lg-12"
                    >
                        {FetchPhases?.isSuccess && !FetchPhases?.isFetching && (
                            <div class="row m-0 w-100">
                                ...
                                <div class="col-lg-12 p-0 mb-3 allcentered">
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            temp.phase_id = props?.callpayload?.phase_id;
                                            AssigntoPhaseMutation?.mutate(temp);
                                        }}
                                    >
                                        {!AssigntoPhaseMutation?.isLoading && <>Done</>}

                                        {AssigntoPhaseMutation?.isLoading && <CircularProgress color="var(--info)" width="20px" height="20px" duration="1s" />}
                                    </button>
                                </div>
                            </div>
                        )}
                            ...
                            <div class="col-lg-12 p-0 mb-3 allcentered">
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        temp.lead_status = props?.callpayload?.leadstatus;
                                        Updateleadstatus?.mutate(temp);
                                    }}
                                >
                                    {!Updateleadstatus?.isLoading && <>Change Status</>}

                                    {Updateleadstatus?.isLoading && <CircularProgress color="var(--info)" width="20px" height="20px" duration="1s" />}
                                </button>
                            </div>
                        </div>
                    </div> */}
                    ...
                    <div class="col-lg-12 p-0 allcentered mt-2">
                        {success?.status && (
                            <div class="text-success text-capitalize" style={{ fontSize: '16px' }}>
                                {success?.type} changed Successfully
                            </div>
                        )}
                    </div>
