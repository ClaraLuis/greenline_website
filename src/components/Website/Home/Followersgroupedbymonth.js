...
const Followersgroupedbymonth = () => {
    ...
    useEffect(() => {
        if (FetchInsights.isSuccess && !FetchInsights.isFetching) {
            var temp = [];
            var tempvalues = [{ name: 'Followers', data: [] }];

            FetchInsights?.data?.data?.followersgroupedbymonth?.map((item, index) => {
...
export default Followersgroupedbymonth;
