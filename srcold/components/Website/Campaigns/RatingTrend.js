    ...
    useEffect(() => {
        if (props?.type == 'line') {
            ...
            props?.data1?.analyisPerDay?.map((item, index) => {
                if (props?.type == '%') {
                    var allvalues =
                        ((parseFloat(props?.data1?.analyisPerDay[index][props?.fetch]) / 100 +
                            parseFloat(props?.data2?.analyisPerDay[index][props?.fetch]) / 100 +
                            parseFloat(props?.data3?.analyisPerDay[index][props?.fetch]) / 100) /
                            3) *
                        100;
                } else {
                    var allvalues =
                        parseFloat(props?.data1?.analyisPerDay[index][props?.fetch]) +
                        parseFloat(props?.data2?.analyisPerDay[index][props?.fetch]) +
                        parseFloat(props?.data3?.analyisPerDay[index][props?.fetch]);
                }
